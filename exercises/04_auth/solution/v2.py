...
Exercise 4 solution: routes also depend on `get_allowed_products` and
filter/404 based on the caller's product scope.
"""
...
def _check_scope(slug: str, allowed: list[str]):
    """Return 404 if the product is outside the caller's scope.

    We return 404 (not 403) so that the existence of out-of-scope products
    isn't leaked to scoped callers — a common pattern for read-only APIs.
    """
...
def list_products(
    ...
    via `limit` and `offset`. Results are filtered to the caller's product scopes.
    """
    stmt = (
        select(Product)
...
def get_product(
    ...
    Returns 404 if no product matches the slug, or if it is outside the
    caller's scope.
    """
    _check_scope(slug, allowed)
    stmt = (
        select(Product)
        .options(
...
# Metric catalog endpoints aren't scoped — they describe definitions, not data.
