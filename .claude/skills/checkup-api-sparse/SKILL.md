---
name: checkup-api-sparse
description: Query data product health metrics via the Checkup API (using a sparse OpenAPI spec)
---
...
1. Fetch the OpenAPI spec first to understand available routes, parameters,
   examples, and response models. **For this task you must use the sparse
   spec snapshot. Do NOT fetch `/openapi.json`**:

   ```bash
   cat exercises/05_agent/openapi_sparse.json
   ```
